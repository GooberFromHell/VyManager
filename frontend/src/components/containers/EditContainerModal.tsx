...
export function EditContainerModal({
  ...
  // ============================================================================
  // Initialize from container prop
  // ============================================================================
  ...
  useEffect(() => {
    if (open && container) {
      ...
      setRestart(container.restart || "no");
      setHostName(container.host_name || "");
      setEntrypoint(container.entrypoint || "");
      setCommand(container.command || "");
      setCommandArgs(container.arguments || "");
      setDisabled(container.disabled);

      setSelectedNetworks(
        container.networks.map((n) => ({
          name: n,
          address: container.network_addresses[n] || "",
        }))
      );
      setPorts(
      ...
      setNameServers([...container.name_servers]);
      setAllowHostNetworks(container.allow_host_networks);
      setAllowHostPid(container.allow_host_pid);
      ...
      setEnvVars(
        container.environment.map((e) => ({ key: e.key, value: e.value }))
      );
      setLabels(
        container.labels.map((l) => ({ key: l.key, value: l.value }))
      );
      setSelectedCapabilities([...container.capabilities]);

      setMemory(container.memory != null ? String(container.memory) : "");
      setCpuQuota(container.cpu_quota || "");
      setUid(container.uid != null ? String(container.uid) : "");
      setGid(container.gid != null ? String(container.gid) : "");
      setDevices(
      ...
      setLogDriver(container.log_driver || "");
      setSysctls(
        container.sysctls.map((s) => ({
  ...
  const addNetwork = () => {
    ...
    if (unusedNetworks.length === 0) return;
    setSelectedNetworks((prev) => [
      ...prev,
      { name: unusedNetworks[0], address: "" },
    ]);
  };
  ...
  // ============================================================================
  // Build diff operations against original container state
  // ============================================================================

  const buildOperations = (): ContainerBatchOperation[] => {
    if (!container) return [];
    const ops: ContainerBatchOperation[] = [];

    // Image
    const newImage = image.trim();
    if (newImage !== (container.image || "")) {
      if (newImage)
        ops.push({ op: "set_container_image", value: newImage });
    }

    // Description
    const newDesc = description.trim();
    const oldDesc = container.description || "";
    if (newDesc !== oldDesc) {
      if (newDesc)
        ops.push({ op: "set_container_description", value: newDesc });
      else ops.push({ op: "delete_container_description" });
    }

    // Restart
    const newRestart = restart;
    const oldRestart = container.restart || "no";
    if (newRestart !== oldRestart) {
      if (newRestart && newRestart !== "no")
        ops.push({ op: "set_container_restart", value: newRestart });
      else ops.push({ op: "delete_container_restart" });
    }

    // Host name
    const newHostName = hostName.trim();
    const oldHostName = container.host_name || "";
    if (newHostName !== oldHostName) {
      if (newHostName)
        ops.push({ op: "set_container_host_name", value: newHostName });
      else ops.push({ op: "delete_container_host_name" });
    }

    // Entrypoint
    const newEntrypoint = entrypoint.trim();
    const oldEntrypoint = container.entrypoint || "";
    if (newEntrypoint !== oldEntrypoint) {
      if (newEntrypoint)
        ops.push({ op: "set_container_entrypoint", value: newEntrypoint });
      else ops.push({ op: "delete_container_entrypoint" });
    }

    // Command
    const newCommand = command.trim();
    const oldCommand = container.command || "";
    if (newCommand !== oldCommand) {
      if (newCommand)
        ops.push({ op: "set_container_command", value: newCommand });
      else ops.push({ op: "delete_container_command" });
    }

    // Arguments
    const newArgs = commandArgs.trim();
    const oldArgs = container.arguments || "";
    if (newArgs !== oldArgs) {
      if (newArgs)
        ops.push({ op: "set_container_arguments", value: newArgs });
      else ops.push({ op: "delete_container_arguments" });
    }

    // Disabled
    if (disabled !== container.disabled) {
      ops.push({
        op: disabled ? "set_container_disable" : "delete_container_disable",
      });
    }

    // Networks — add new, remove deleted
    const oldNetworkNames = new Set(container.networks);
    const newNetworkNames = new Set(
      selectedNetworks.filter((n) => n.name).map((n) => n.name)
    );
    for (const net of selectedNetworks) {
      if (!net.name) continue;
      if (!oldNetworkNames.has(net.name)) {
        ops.push({ op: "set_container_network", value: net.name });
      }
      const oldAddress = container.network_addresses[net.name] || "";
      if (net.address !== oldAddress) {
        if (net.address)
          ops.push({
          ...
          ops.push({
            op: "delete_container_network_address",
            value: net.name,
          });
      }
    }
    for (const n of oldNetworkNames) {
      if (!newNetworkNames.has(n)) {
        ops.push({ op: "delete_container_network", value: n });
      }
    }

    // Ports — full replace strategy: delete all old, set all new
    for (const p of container.ports) {
      ops.push({ op: "delete_container_port", value: p.name });
    }
    ...
    // Name servers
    const oldNS = new Set(container.name_servers);
    const newNS = new Set(nameServers.filter((n) => n.trim()));
    for (const ns of newNS) {
      if (!oldNS.has(ns))
        ops.push({ op: "set_container_name_server", value: ns });
    }
    for (const ns of oldNS) {
      if (!newNS.has(ns))
        ops.push({ op: "delete_container_name_server", value: ns });
    }

    // Allow host networks
    if (allowHostNetworks !== container.allow_host_networks) {
      ops.push({
        op: allowHostNetworks
          ? "set_container_allow_host_networks"
          : "delete_container_allow_host_networks",
      });
    }

    // Allow host PID
    if (allowHostPid !== container.allow_host_pid) {
      ops.push({
        op: allowHostPid
          ? "set_container_allow_host_pid"
          : "delete_container_allow_host_pid",
      });
    }

    // Volumes — full replace strategy
    for (const v of container.volumes) {
      ops.push({ op: "delete_container_volume", value: v.name });
    }
    ...
    // Tmpfs (v1.5 only) — full replace strategy
    if (capabilities?.has_tmpfs) {
      for (const t of container.tmpfs_mounts) {
        ops.push({ op: "delete_container_tmpfs", value: t.name });
      }
    ...
    // Environment — full replace strategy
    for (const e of container.environment) {
      ops.push({ op: "delete_container_environment", value: e.key });
    }
    ...
    // Labels — full replace strategy
    for (const l of container.labels) {
      ops.push({ op: "delete_container_label", value: l.key });
    }
    ...
    // Capabilities
    const oldCaps = new Set(container.capabilities);
    const newCaps = new Set(selectedCapabilities);
    for (const cap of newCaps) {
      if (!oldCaps.has(cap))
        ops.push({ op: "set_container_capability", value: cap });
    }
    for (const cap of oldCaps) {
      if (!newCaps.has(cap))
        ops.push({ op: "delete_container_capability", value: cap });
    }

    // Memory
    const newMemory = memory ? Number(memory) : null;
    if (newMemory !== container.memory) {
      if (newMemory != null)
        ops.push({ op: "set_container_memory", value: String(newMemory) });
      else ops.push({ op: "delete_container_memory" });
    }

    // CPU quota
    const newCpuQuota = cpuQuota.trim();
    const oldCpuQuota = container.cpu_quota || "";
    if (newCpuQuota !== oldCpuQuota) {
      if (newCpuQuota)
        ops.push({ op: "set_container_cpu_quota", value: newCpuQuota });
      else ops.push({ op: "delete_container_cpu_quota" });
    }

    // UID
    const newUid = uid ? Number(uid) : null;
    if (newUid !== container.uid) {
      if (newUid != null)
        ops.push({ op: "set_container_uid", value: String(newUid) });
      else ops.push({ op: "delete_container_uid" });
    }

    // GID
    const newGid = gid ? Number(gid) : null;
    if (newGid !== container.gid) {
      if (newGid != null)
        ops.push({ op: "set_container_gid", value: String(newGid) });
      else ops.push({ op: "delete_container_gid" });
    }

    // Devices — full replace strategy
    for (const d of container.devices) {
      ops.push({ op: "delete_container_device", value: d.name });
    }
    ...
    if (capabilities?.has_log_driver) {
      const newLogDriver = logDriver;
      const oldLogDriver = container.log_driver || "";
      if (newLogDriver !== oldLogDriver) {
        if (newLogDriver)
          ops.push({ op: "set_container_log_driver", value: newLogDriver });
        else ops.push({ op: "delete_container_log_driver" });
      }
    }

    // Sysctl (v1.5 only) — full replace strategy
    if (capabilities?.has_sysctl) {
      for (const s of container.sysctls) {
        ops.push({ op: "delete_container_sysctl", value: s.parameter });
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await containerService.updateContainer(container.name, operations);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] !flex !flex-col overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>
            Modify the configuration for this container.
          </DialogDescription>
        </DialogHeader>
              ...
              <TabsContent value="general" className="space-y-5 pt-2">
                <Fieldset label="Identity">
                  <FormField
                    ...
                    description="Container name cannot be changed after creation"
                  >
                    <Input
                      id="container-name"
                    ...
                    {nameServers.map((ns, i) => (
                      <div key={i} className="flex gap-2">
                        <Input
                          value={ns}
                          onChange={(e) =>
                            updateNameServer(i, e.target.value)
                          }
              ...
              <TabsContent value="advanced" className="space-y-5 pt-2">
                ...
                {capabilities?.has_log_driver && (
                  <>
                    ...
                    <Fieldset label="Logging" description="VyOS 1.5+ only">
                      <FormField
                        label="Log Driver"
