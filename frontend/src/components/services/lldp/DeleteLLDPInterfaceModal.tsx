...
export function DeleteLLDPInterfaceModal({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError((err as ApiError).message || "Failed to delete LLDP interface");
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Delete LLDP Interface
          </DialogTitle>
          <DialogDescription>
            This will permanently remove this interface from the LLDP configuration.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-4">
            <div className="space-y-2 text-sm">
              ...
              <div className="space-y-1 text-muted-foreground">
                <p>
                  <span className="font-medium">Interface:</span> {interfaceName}
                </p>
              </div>
            </div>
          </div>
