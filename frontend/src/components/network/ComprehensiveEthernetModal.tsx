...
const STEPS = [
  { id: "basic",    title: "Basic",     description: "Name, addresses, speed, MTU" },
  { id: "advanced", title: "Advanced",  description: "Offload, ring buffer, TCP MSS, flow" },
  { id: "ip",       title: "IP / IPv6", description: "ARP, source validation, IPv6 options" },
  { id: "dhcp",     title: "DHCP",      description: "DHCPv4 and DHCPv6 options" },
  { id: "special",  title: "Special",   description: "Mirroring, EAPoL, EVPN" },
];
...
export function ComprehensiveEthernetModal({
  ...
  useEffect(() => {
    ...
    setError(null);
    setStep(0); // always start at first step when (re-)opening
  }, [iface, mode, open]);
  ...
  const buildOperations = (): BatchOperation[] => {
    ...
    for (const addr of newAddrs) {
      if (!currentAddrs.has(addr)) operations.push({ op: "set_address", value: addr });
    }
    if (mode === "edit") {
      for (const addr of currentAddrs) {
        if (!newAddrs.has(addr)) operations.push({ op: "delete_address", value: addr });
      }
    }
    ...
    if (capabilities?.features.offload) {
      const handleOffload = (
        ...
        if (mode === "create") {
          if (nv === "on") operations.push({ op: setOp });
        } else if (mode === "edit" && cv !== nv) {
          if (nv === "on") operations.push({ op: setOp });
          else if (nv === "off" && cv === "on") operations.push({ op: deleteOp });
        }
      };

      handleOffload(iface?.offload?.gro, offloadGro, "set_offload_gro", "delete_offload_gro");
      handleOffload(iface?.offload?.gso, offloadGso, "set_offload_gso", "delete_offload_gso");
      handleOffload(iface?.offload?.lro, offloadLro, "set_offload_lro", "delete_offload_lro");
      handleOffload(iface?.offload?.rps, offloadRps, "set_offload_rps", "delete_offload_rps");
      handleOffload(iface?.offload?.sg,  offloadSg,  "set_offload_sg",  "delete_offload_sg");
      handleOffload(iface?.offload?.tso, offloadTso, "set_offload_tso", "delete_offload_tso");
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "create") {
        ...
        await ethernetService.batchConfigure({ interface: interfaceName, operations: buildOperations() });
      } else {
        const ops = buildOperations();
        if (ops.length === 0) { setError("No changes detected"); setLoading(false); return; }
        await ethernetService.updateInterface(iface!.name, ops);
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        ...
        <Stepper steps={STEPS} currentIndex={step} onGoTo={setStep}>
          ...
          <StepperContent stepIndex={0}>
              ...
              <div className="space-y-2">
                ...
                {addresses.map((address, index) => (
                  <div key={index} className="flex gap-2">
                    ...
                    />
                    {addresses.length > 0 && (
                      <Button type="button" variant="outline" size="sm" onClick={() => handleRemoveAddress(index)}>
                        <X className="h-4 w-4" />
                      </Button>
                    )}
                  </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {capabilities?.features.ethernet.speed && (
                <div className="space-y-2">
                  ...
                  <Select value={speed || "auto"} onValueChange={(v) => setSpeed(v === "auto" ? "" : v)}>
                    <SelectTrigger id="speed"><SelectValue placeholder="Auto" /></SelectTrigger>
                    <SelectContent>
              ...
              {capabilities?.features.ethernet.duplex && (
                <div className="space-y-2">
                  ...
                  <Select value={duplex || "auto"} onValueChange={(v) => setDuplex(v === "auto" ? "" : v)}>
                    <SelectTrigger id="duplex"><SelectValue placeholder="Auto" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="auto">Auto</SelectItem>
              ...
              {iface?.hw_id && mode === "edit" && (
                <div className="space-y-2">
                  ...
                  <Input id="hw-id" value={iface.hw_id} disabled className="font-mono" />
                </div>
              )}
            ...
            {capabilities?.features.basic.disable && (
              <div className="flex items-center space-x-2">
                <Checkbox id="disable" checked={disabled} onCheckedChange={(c) => setDisabled(c as boolean)} />
                <Label htmlFor="disable" className="cursor-pointer">Administratively disable interface</Label>
              </div>
            )}
          </StepperContent>
            ...
            {capabilities?.features.offload && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {(["GRO","GSO","LRO","RPS","SG","TSO"] as const).map((label) => {
                    const key = label.toLowerCase() as "gro"|"gso"|"lro"|"rps"|"sg"|"tso";
                    const vals: Record<string, string> = { gro: offloadGro, gso: offloadGso, lro: offloadLro, rps: offloadRps, sg: offloadSg, tso: offloadTso };
                    const setters: Record<string, (v: string) => void> = { gro: setOffloadGro, gso: setOffloadGso, lro: setOffloadLro, rps: setOffloadRps, sg: setOffloadSg, tso: setOffloadTso };
                    return (
                      <div key={key} className="space-y-2">
                        <Label htmlFor={`offload-${key}`}>{label}</Label>
                        <Select value={vals[key]} onValueChange={setters[key]}>
                          <SelectTrigger id={`offload-${key}`}><SelectValue placeholder="Select..." /></SelectTrigger>
                          <SelectContent>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input id="ring-rx" type="number" placeholder="256" value={ringBufferRx} onChange={(e) => setRingBufferRx(e.target.value)} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="ring-tx">TX Buffer</Label>
                    <Input id="ring-tx" type="number" placeholder="256" value={ringBufferTx} onChange={(e) => setRingBufferTx(e.target.value)} />
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input id="ip-mss" type="number" placeholder="1460" value={ipAdjustMss} onChange={(e) => setIpAdjustMss(e.target.value)} disabled={ipClampMssToPmtu} />
                    <div className="flex items-center space-x-2">
                      <Checkbox id="ip-clamp-pmtu" checked={ipClampMssToPmtu} onCheckedChange={(c) => setIpClampMssToPmtu(c as boolean)} />
                      <Label htmlFor="ip-clamp-pmtu" className="cursor-pointer text-xs">Clamp to PMTU</Label>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="ipv6-mss">IPv6 Adjust MSS</Label>
                    <Input id="ipv6-mss" type="number" placeholder="1440" value={ipv6AdjustMss} onChange={(e) => setIpv6AdjustMss(e.target.value)} disabled={ipv6ClampMssToPmtu} />
                    <div className="flex items-center space-x-2">
                      <Checkbox id="ipv6-clamp-pmtu" checked={ipv6ClampMssToPmtu} onCheckedChange={(c) => setIpv6ClampMssToPmtu(c as boolean)} />
                      <Label htmlFor="ipv6-clamp-pmtu" className="cursor-pointer text-xs">Clamp to PMTU</Label>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {capabilities?.features.flow_control && (
                    <div className="flex items-center space-x-2">
                      <Checkbox id="disable-flow-control" checked={disableFlowControl} onCheckedChange={(c) => setDisableFlowControl(c as boolean)} />
                      <Label htmlFor="disable-flow-control" className="cursor-pointer">Disable Flow Control</Label>
                    </div>
                  )}
                  {capabilities?.features.link_detect && (
                    <div className="flex items-center space-x-2">
                      <Checkbox id="disable-link-detect" checked={disableLinkDetect} onCheckedChange={(c) => setDisableLinkDetect(c as boolean)} />
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input id="arp-cache-timeout" type="number" placeholder="30" value={arpCacheTimeout} onChange={(e) => setArpCacheTimeout(e.target.value)} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { id: "arp-disable-filter",   label: "Disable ARP Filter",   checked: arpDisableFilter,   setter: setArpDisableFilter },
                    { id: "arp-enable-accept",     label: "Enable ARP Accept",    checked: arpEnableAccept,    setter: setArpEnableAccept },
                    { id: "arp-enable-announce",   label: "Enable ARP Announce",  checked: arpEnableAnnounce,  setter: setArpEnableAnnounce },
                    { id: "arp-enable-ignore",     label: "Enable ARP Ignore",    checked: arpEnableIgnore,    setter: setArpEnableIgnore },
                    { id: "arp-enable-proxy",      label: "Enable Proxy ARP",     checked: arpEnableProxyArp,  setter: setArpEnableProxyArp },
                    { id: "arp-proxy-pvlan",       label: "Proxy ARP PVLAN",      checked: arpProxyArpPvlan,   setter: setArpProxyArpPvlan },
                  ].map(({ id, label, checked, setter }) => (
                    <div key={id} className="flex items-center space-x-2">
                      <Checkbox id={id} checked={checked} onCheckedChange={(c) => setter(c as boolean)} />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select value={ipSourceValidation || "none"} onValueChange={(v) => setIpSourceValidation(v === "none" ? "" : v)}>
                      <SelectTrigger id="ip-source-validation"><SelectValue placeholder="None" /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="flex items-center space-x-2 pt-8">
                    <Checkbox id="ip-directed-broadcast" checked={ipEnableDirectedBroadcast} onCheckedChange={(c) => setIpEnableDirectedBroadcast(c as boolean)} />
                    <Label htmlFor="ip-directed-broadcast" className="cursor-pointer text-sm">Enable Directed Broadcast</Label>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="ipv6-dad" type="number" placeholder="1" value={ipv6DupAddrDetectTransmits} onChange={(e) => setIpv6DupAddrDetectTransmits(e.target.value)} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="flex items-center space-x-2">
                    <Checkbox id="ipv6-autoconf" checked={ipv6Autoconf} onCheckedChange={(c) => setIpv6Autoconf(c as boolean)} />
                    <Label htmlFor="ipv6-autoconf" className="cursor-pointer text-sm">Enable Autoconfig</Label>
                  </div>
                  <div className="flex items-center space-x-2">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="dhcp-vendor-class" placeholder="vendor-class" value={dhcpVendorClassId} onChange={(e) => setDhcpVendorClassId(e.target.value)} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="dhcp-route-distance">Default Route Distance</Label>
                    <Input id="dhcp-route-distance" type="number" placeholder="210" value={dhcpDefaultRouteDistance} onChange={(e) => setDhcpDefaultRouteDistance(e.target.value)} />
                  </div>
                </div>
                <div className="flex items-center space-x-2">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input id="dhcpv6-duid" placeholder="00:01:00:01:12:34:56:78:9a:bc:de:f0" value={dhcpv6Duid} onChange={(e) => setDhcpv6Duid(e.target.value)} />
                  </div>
                </div>
                <div className="flex items-center space-x-2">
          ...
          <StepperContent stepIndex={4}>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input id="mirror-ingress" placeholder="eth0" value={mirrorIngress} onChange={(e) => setMirrorIngress(e.target.value)} />
                  </div>
                  <div className="space-y-2">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input id="eapol-ca-cert" placeholder="/config/auth/ca.pem" value={eapolCaCertFile} onChange={(e) => setEapolCaCertFile(e.target.value)} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="eapol-cert">Certificate File</Label>
                    <Input id="eapol-cert" placeholder="/config/auth/cert.pem" value={eapolCertFile} onChange={(e) => setEapolCertFile(e.target.value)} />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="eapol-key">Key File</Label>
                    <Input id="eapol-key" placeholder="/config/auth/key.pem" value={eapolKeyFile} onChange={(e) => setEapolKeyFile(e.target.value)} />
                  </div>
                </div>
              </div>
            )}
            ...
            {capabilities?.features.evpn?.uplink_tracking && (
              <div className="space-y-3">
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox id="evpn-uplink" checked={evpnUplink} onCheckedChange={(c) => setEvpnUplink(c as boolean)} />
                  <Label htmlFor="evpn-uplink" className="cursor-pointer text-sm">Enable EVPN Uplink Tracking</Label>
                </div>
              </div>
            )}
          </StepperContent>
          ...
          <StepperNavigation
            ...
            submitLabel={mode === "create" ? "Create Interface" : "Save Changes"}
